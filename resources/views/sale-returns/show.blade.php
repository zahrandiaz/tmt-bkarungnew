<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Detail Retur Penjualan #{{ $saleReturn->return_code }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                        <a href="{{ route('sale-returns.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-800 uppercase tracking-widest hover:bg-gray-300 mb-3 md:mb-0">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6 border-b pb-6">
                        <div>
                            ...
                            <p class="mt-1 text-lg font-semibold text-gray-900">{{ $saleReturn->return_code }}</p>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Referensi Invoice Penjualan</h3>
                            <a href="{{ route('sales.show', $saleReturn->sale->id) }}" class="mt-1 text-lg font-semibold text-blue-600 hover:underline">{{ $saleReturn->sale->invoice_number }}</a>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Tanggal & Waktu Retur</h3>
                            <p class="mt-1 text-lg font-semibold text-gray-900">{{ \Carbon\Carbon::parse($saleReturn->return_date)->isoFormat('D MMM YYYY, HH:mm') }}</p>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Dicatat Oleh</h3>
                            <p class="mt-1 text-lg font-semibold text-gray-900">{{ $saleReturn->user->name ?? 'N/A' }}</p>
                        </div>
                    </div>

                    <div class="mb-6">
                        <h3 class="text-sm font-medium text-gray-500">Catatan Retur</h3>
                        <p class="mt-1 text-gray-700">{{ $saleReturn->notes ?? 'Tidak ada catatan.' }}</p>
                    </div>
                        ...
                        <table class="w-full text-sm text-left text-gray-500">
                            ...
                            <tfoot class="font-bold text-gray-900">
                                <tr>
                                    ...
                                    <td class="px-6 py-3 text-right">{{ 'Rp ' . number_format($saleReturn->total_amount, 0, ',', '.') }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>

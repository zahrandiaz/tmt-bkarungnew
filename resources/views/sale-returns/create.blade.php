            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900"
                     x-data="{
                        ...
                        initTomSelect() {
                            let tomSelect = new TomSelect(this.$refs.saleSearch, {
                                maxItems: 1,
                                valueField: 'id',
                                labelField: 'text',
                                searchField: 'text',
                                load: (query, callback) => {
                                    ...
                                    fetch(`{{ route('api.sales.search') }}?q=${encodeURIComponent(query)}`)
                        ...
                        fetchSaleDetails() {
                            if (!this.formData.sale_id) {
                                this.items = [];
                                this.calculateTotal();
                                return;
                            }
                            fetch(`/api/sales/${this.formData.sale_id}/details`)
                                .then(response => response.json())
                                .then(data => {
                                    ...
                                    .map(detail => ({
                                        ...
                                        unit_price: parseFloat(detail.sale_price) || 0,
                        ...
                        submitForm() {
                            ...
                            fetch('{{ route('sale-returns.store') }}', {
                                method: 'POST',
                                headers: {
                    ...
                    <form @submit.prevent="submitForm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <x-input-label for="sale_id" :value="__('Cari Invoice Penjualan')" />
                                <input x-ref="saleSearch" id="sale_id" name="sale_id" type="text" placeholder="Ketik No. Invoice atau Nama Pelanggan..." class="block mt-1 w-full">
                        ...
                        <div class="mt-8 border-t pt-6 text-center text-gray-500" x-show="formData.sale_id && items.length === 0">
                            Semua item dari transaksi ini sudah diretur sepenuhnya.
                        </div>

                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('sale-returns.index') }}" class="text-sm text-gray-600 hover:text-gray-900 mr-4">Batal</a>
                            <x-primary-button>
                                {{ __('Simpan Retur') }}
                            </x-primary-button>
                        </div>

            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-1">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            ...
                            <form method="POST" action="{{ route('receivables.payments.store', $sale) }}" enctype="multipart/form-data">
                                @csrf
                                <div class="mt-4">
                ...
                <div class="@can('finance-manage-payment') md:col-span-2 @else md:col-span-3 @endcan">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                        <div class="p-6">
                            <h3 class="text-lg font-medium text-gray-900 mb-4">Ringkasan Piutang</h3>
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between"><span class="text-gray-500">No. Invoice:</span> <span class="font-semibold">{{ $sale->invoice_number }}</span></div>
                                <div class="flex justify-between"><span class="text-gray-500">Pelanggan:</span> <span class="font-semibold">{{ $sale->customer->name }}</span></div>
                                <div class="flex justify-between"><span class="text-gray-500">Tanggal Transaksi:</span> <span class="font-semibold">{{ \Carbon\Carbon::parse($sale->sale_date)->format('d M Y, H:i') }}</span></div>
                                <hr class="my-2">
                                <div class="flex justify-between text-base"><span class="text-gray-500">Total Tagihan:</span> <span class="font-bold">Rp {{ number_format($sale->total_amount, 0, ',', '.') }}</span></div>
                                <div class="flex justify-between"><span class="text-gray-500">Sudah Dibayar:</span> <span class="font-semibold text-green-600">Rp {{ number_format($sale->total_paid, 0, ',', '.') }}</span></div>
                                <div class="flex justify-between text-lg"><span class="text-gray-500">Sisa Tagihan:</span> <span class="font-bold text-red-600">Rp {{ number_format($sale->total_amount - $sale->total_paid, 0, ',', '.') }}</span></div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-6">
                            ...
                            <div class="relative overflow-x-auto">
                                <table class="w-full text-sm text-left text-gray-500">
                                    ...
                                    <tbody>
                                        @forelse ($sale->payments->sortByDesc('payment_date') as $payment)
